const stealCakes = (cakes, capacity) => {
    cakes.map(cake => cake.factor = cake.value / cake.weight)
    const sortedCakes = cakes.sort((a, b) => a.factor < b.factor)
    let spaceUsed = 0
    let maxLoot = 0

    while (spaceUsed <= capacity && sortedCakes.length) {
        if (sortedCakes[0].weight + spaceUsed <= capacity) {
            maxLoot += sortedCakes[0].value
            spaceUsed += sortedCakes[0].weight
        } else {
            sortedCakes.shift()
        }
    }
...
console.log(stealCakes(cakeTypes, capacity))